...
func (sql sQLStruct) InitDB() error {
	dsn := "blog_go:Y73sT2kkMjYczxRn@tcp(192.168.199.145:3306)/blog_go?charset=utf8mb4&parseTime=True&loc=Local"
...
func (sql sQLStruct) getCommentsPre(id uint) (returnData GetCommentsReturn) {
	...
	result := db.Where("id > ?", fmt.Sprint(id)).Limit(CommentPageSize).Find(&comments)
	if result.Error != nil {
...
func (sql sQLStruct) getCommentsPageNext(id uint) (returnData GetCommentsReturn) {
	comments := make([]CommentStruct, 0)
	result := db.Where("id < ?", fmt.Sprint(id)).Order("id desc").Limit(CommentPageSize).Find(&comments)
	if result.Error != nil {
		returnData.Errmsg = "find comments failed"
		returnData.Haserr = true
	} else if result.RowsAffected == 0 {
